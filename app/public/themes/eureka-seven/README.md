...
本目录是 `fogg_Bocchi-the-Rock` 独立版本保留的早期可拆卸主题资产层。素材只在本地运行时读取，不会向素材站点发出网络请求。

## 使用范围

- 用于独立动漫版本的本地运行、构建与界面回溯。
- 不把角色设为奖励、惩罚、打卡监督者或“失败”反馈。
- 仓库保留素材副本、官方来源、取得日期和文件校验，确保克隆后可以还原完整主题。
...
3. 若要替换主题，移除该区域和本目录；基础界面仍有纯色/本地通用背景作为回退。
