...
V0.3 是当前正式版本。它在 V0.2 双平台、三类工作流的基础上，合并了协作者的“孤独摇滚”视觉分支：专业模式仍为默认主题，用户可在“数据与外观”中切换动漫主题。两种主题共享同一套行为模型、SQLite 数据结构和业务流程，只改变视觉表达。

Windows 与 Android 共享核心业务代码，但数据分别保存在各自设备本地，不会自动同步。主题偏好也只保存在当前设备，不上传、不写入行为数据备份。
...
- 专业模式与“孤独摇滚”动漫主题即时切换；桌面与移动端共用同一主题偏好。
...
V0.1 与 V0.2 的标签和发布记录继续保留；V0.3 起在同一 Windows / Android 产品内同时维护专业与动漫主题。

## 主题资源说明

“孤独摇滚”主题由协作者分支合并而来，角色图片与标志的来源、原始链接和版权边界记录在 [主题资源说明](app/public/themes/kessoku/README.md)。这些资源不代表本项目获得了商业再分发授权；在公开发布、商用或上传应用商店前，仓库所有者应单独确认授权，必要时替换为自有资源。专业模式完全使用项目自有的离线界面资产。
